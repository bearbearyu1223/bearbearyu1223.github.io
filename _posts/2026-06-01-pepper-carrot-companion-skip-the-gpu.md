...
- **The prompts** (`backend/app/core/prompts.py`) — unchanged. The [Post 11 hardening]({% post_url 2026-05-30-pepper-carrot-companion-prompt-hardening %}) that taught a small model to be concise and stay grounded still applies; a bigger model just clears the bar more easily.
...
Those strings are read by a factory that's existed since Post 4. Here's the actual code that does the dispatch — note that adding the managed-API path required adding *branches*, not changing callers:
...
Everything else — Neon provisioning, R2 upload, the Pages connect-and-build, the `CORS_ORIGINS` round-trip — is byte-for-byte Posts 14–15. When you open the `*.pages.dev` URL and ask a question, **the first answer is fast** — no GPU to wake. That speed *is* the demo for this path, the same way the choreographed cold start was the demo for Posts 14–15's.
