...
- **The "don't refer to the notes as user-provided" rule** is what stops the model from saying things like *"From your cast list above, the character on this page is Pepper."* That phrasing is technically correct, but it tears the fourth wall of the demo, since the user never *wrote* a cast list. Telling the model not to acknowledge its scaffolding keeps the answer feeling like a friend reading along, not like an LLM reading a brief.
...
- **"Brevity is not a tradeoff against quality here, it IS the quality."** This one is a deliberate reframe. A 7B model under pressure thinks longer = more helpful; saying the inverse explicitly, in italics, in the prompt, moves it. Treat the prompt like a one-shot lesson: explain the *why*, not just the *what*.

The page-mode equivalent is the per-question focus rule (*"Focus on what's happening on the current spread and the immediate story"*), the same idea from a different angle. Both are mode-specific because the kinds of *length-failure* are mode-specific: page mode tends to invent panel detail to pad the answer; wiki mode tends to tour the article.
...
The structural validation in `_parse_suggestions` ([Post 10]({% post_url 2026-05-25-pepper-carrot-companion-streaming-chat %}#validation)) still runs after this, so chips that *do* slip through as a statement or a truncated fragment get dropped on the server. The schema guarantees parseability; the prompt examples guarantee *taste*; the validator catches what slips past both. Three layers of defense, none of them sufficient alone.
...
User bubbles stay plain text: the user typed it, and if they put markdown in their question, they meant it. Assistant bubbles go through `ReactMarkdown`. The CSS in `global.css` tones down the browser defaults, since a stray `### header` shouldn't render at h3 *document* sizing in a chat bubble:
...
One picture of where each tightener lives in the pipeline. Notice that *only* the prompt-bound paths gain anything: retrieval, the spoiler filter, the chip schema, and the SSE framing are all from Posts 9–10 and unchanged.
