...
- Wang, Hayou and Nalisnick, [The Myth of Expert Specialization in MoEs: Why Routing Reflects Geometry, Not Necessarily Domain Expertise](https://arxiv.org/abs/2604.09780) (2026) — the dissenting reading of §4. Argues that because routers are linear maps, specialization is emergent from the representation space. Read Proposition 1 rather than the abstract on this point: the abstract says hidden-state similarity is "necessary and sufficient" to explain expert-usage similarity, but the proposition is a one-directional bound, and the text is explicit that "when the hidden states differ, their router logits can be either similar or dissimilar." Their five models are GPT-OSS-20B, ERNIE-4.5-21B, Qwen-3-30B, Ling-mini and Trinity-Mini-Base; OLMoE is cited (its Figure 22) but not measured. Their result on load balancing also bears on §8: Proposition 2 proves the balancing loss suppresses shared hidden-state directions. Their expert-pruning experiment (Figure 8) is §4's example of an intervention.
- Hu et al., [What Gets Activated: Uncovering Domain and Driver Experts in MoE Language Models](https://arxiv.org/abs/2601.10159) (2026) — §4's example of causal-effect evidence about individual experts, as opposed to the routing counts this post measures.
