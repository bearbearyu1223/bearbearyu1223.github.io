...
The three posts since then each cut one of three costs of running a trained model: **how much arithmetic** it does, **how much memory** it holds, and **how many bytes** it has to move around inside the hardware while doing that arithmetic. They also differ in what they did to the output. The KV cache left it untouched: generating with and without it gives identical tokens. Flash Attention computes the same function, but adds the same numbers in a different order, which moves the result in its last bits. Quantization computes a different function on purpose.
