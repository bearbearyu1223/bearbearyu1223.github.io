...
**The `Annotated[..., Depends(...)]` alias pattern.** Declaring `SessionDep` and `StorageDep` at module scope reuses the dependency wiring across both routes without repeating `Depends(...)` at every signature. Pure ergonomic win — same behavior as inlining `db: AsyncSession = Depends(get_session)`, half the visual noise. (See FastAPI's [Annotated dependencies docs](https://fastapi.tiangolo.com/tutorial/dependencies/#share-annotated-dependencies) for the canonical pattern.)
