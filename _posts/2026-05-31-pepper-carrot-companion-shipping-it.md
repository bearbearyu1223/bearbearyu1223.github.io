...
- **`asyncio.to_thread` wraps every network call.** boto3 is synchronous. FastAPI is async. Mixing them naively — `self._client.put_object(...)` inside an async route handler — blocks the event loop for the duration of the upload, and a slow upload can starve every other inbound request. `await asyncio.to_thread(_put)` parks the blocking call on a worker thread and yields the event loop. The pattern is a one-liner because the abstraction lets it be.
