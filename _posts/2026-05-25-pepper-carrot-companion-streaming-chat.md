...
description: >-
  Post 10 of the Pepper & Carrot AI flipbook series. Post 9 left a
...
>
> **Prerequisites.**
> - The workshop starter at the [`post-10-streaming` tag](https://github.com/bearbearyu1223/pepper-carrot-companion-workshop/tree/post-10-streaming): `git checkout post-10-streaming`. Everything [Post 9]({% post_url 2026-05-25-pepper-carrot-companion-spoiler-safe-rag %}) needed — Postgres up, migrations applied, the roster seeded, Episode 1 ingested — plus Ollama running with `qwen2.5:7b` and `bge-m3` pulled.
> - [Node.js 20+](https://nodejs.org/) for the Vite frontend, as in [Post 8]({% post_url 2026-05-24-pepper-carrot-companion-flipbook-ui %}).
...
And the retrieved half stays **spoiler-bounded**: on page 3 it can reach pages 1-2, never page 4 and beyond. Future pages simply aren't in the candidate set — the boundary from [Post 9]({% post_url 2026-05-25-pepper-carrot-companion-spoiler-safe-rag %}) is still doing its job underneath the stream.
...
In Post 9 the orchestrator had a method `answer()` that did the whole pipeline and `return`ed a finished string. Post 10 turns that single `return` into a *stream of yields*. The method becomes `stream_response()`, an **async generator**.
...
The orchestrator yields plain dicts; something has to turn them into `event:`/`data:` wire frames. That's the route's job, and we don't hand-roll it — [`sse-starlette`](https://github.com/sysid/sse-starlette) (already a dependency from Post 9) provides `EventSourceResponse`:
...
This closes the loop with [Post 9]({% post_url 2026-05-25-pepper-carrot-companion-spoiler-safe-rag %}): the `PATCH` updates `chat_sessions.current_page`, and that column is exactly what the spoiler filter reads. So as the reader moves through the comic, the set of pages the chat can retrieve grows with them — automatically, and without the chat request ever carrying a page number. The boundary stays server-side; the browser just reports where the reader is.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1180 600" role="img"
  ...
  <g>
    ...
    <text x="735" y="498" text-anchor="middle" font-size="10" fill="#475569" font-style="italic">the spoiler filter (Post 9) reads it — the boundary rides underneath, unchanged</text>
  </g>
