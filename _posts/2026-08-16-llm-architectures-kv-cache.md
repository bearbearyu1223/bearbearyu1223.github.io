...
- **Generating a token costs about two orders of magnitude more than reading one** — 83× per token here. Both phases stream the same weights; prefill spreads that read across the whole prompt, decode pays it in full for a single token. Decode ends up **306× short** of what the hardware needs to keep its arithmetic units busy, so it waits on memory instead of computing. No kernel closes a gap that size — and it's why every LLM API bills output above input, a flat 5× across every Claude tier.
